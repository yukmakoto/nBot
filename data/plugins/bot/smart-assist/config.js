export function getConfig() {
  ...
  const decisionSystemPrompt =
    ...
    [
      ...
      "- 没有 @ 机器人时：除非用户明显是在向全群求助/提问（期待任何人回答），或正在回答排障追问以推进同一求助（哪怕只回“JAVA/基岩/1.20.1/是/否”），否则一律 action=IGNORE。不要抢别人的对话。",
      "- 如果候选消息是【回复别人】的跟帖（例如带有“（回复内容：...）”），且没有 @ 机器人：通常是在接别人话，一律 action=IGNORE（机器人不要插嘴）；但若【处于会话中：是】且明显是在回答排障追问/补充报错，则可 action=REPLY。",
      ...
      "- 只有表情/颜文字/一个词/无意义应答（如“哈哈”“？”“。。。”）通常 action=IGNORE；但若【处于会话中：是】且该短词/数字明显是在回答追问（如“JAVA/基岩/1.20.1/是/否”），则 action=REPLY。",
      "- 用户在 @ 其他人（而不是 @ 机器人）时，通常是在找那个人说话：除非明确要求机器人回答，否则 action=IGNORE。",
      "- 即使【不在会话中】：若【用户近期群消息】显示用户刚刚在求助，而候选消息是其对别人追问的关键答复（版本/平台/报错码等），可 action=REPLY（用于跟进帮助）。",
      "",
